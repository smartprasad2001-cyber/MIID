...
def generate_candidates(original: str, max_depth: int = 10, max_candidates: int = 10000000) -> List[str]:
    """
...
def generate_light_variations(original: str, count: int, verbose: bool = False) -> List[str]:
    ...
    max_depth = 1
    max_candidates = 500000  # Start with even more candidates
    
    # Generate MANY more candidates than needed for Light (harder to find perfect matches)
    target_candidates = count * 500  # Generate 500x more for Light
    
    while len(perfect_matches) < count and max_depth <= 10:  # Increased to depth 10
        if verbose:
        ...
        if len(perfect_matches) < count:
            max_depth += 1
            max_candidates = min(max_candidates * 2, 10000000)  # Very high limit - try everything
        else:
...
def generate_medium_variations(original: str, count: int, target_score: float = 0.7, tolerance: float = 0.25, verbose: bool = False) -> List[str]:
    """
    Generate Medium variations (0.6-0.79 range) using algorithm weights.
    Strategy: Match 2 out of 3 algorithms to get Medium score (sum of 2 weights = 0.6-0.79)
    """
    selected_algorithms, weights = get_weights_for_name(original)
    target_codes = get_algorithm_codes(original)
    
    # Calculate target score ranges based on weights
    # Medium range: 0.6-0.79, so we need 2 out of 3 algorithms to match
    # Try all combinations of 2 algorithms
    algorithm_combinations = []
    for i in range(len(selected_algorithms)):
        for j in range(i+1, len(selected_algorithms)):
            # Score if these 2 algorithms match
            combo_score = weights[i] + weights[j]
            if 0.6 <= combo_score <= 0.79:
                algorithm_combinations.append(([i, j], combo_score))
    
    # If no 2-algorithm combo gives Medium, try single algorithm with highest weight
    if not algorithm_combinations:
        # Find algorithm with highest weight that gives Medium range
        for i, weight in enumerate(weights):
            if 0.6 <= weight <= 0.79:
                algorithm_combinations.append(([i], weight))
    
    medium_range_candidates = []
    tested: Set[str] = set()
    ...
    while len(medium_range_candidates) < count and max_depth <= 10:
        if verbose:
        ...
        # Check each candidate against target algorithm combinations
        for var in candidates:
            if var == original or var in tested:
                continue
            ...
            # Check if this variation matches any of our target algorithm combinations
            for combo_indices, target_score_val in algorithm_combinations:
                matches = True
                for idx in combo_indices:
                    algo_name = selected_algorithms[idx]
                    if algo_name == 'soundex':
                        if var_codes['soundex'] != target_codes['soundex']:
                            matches = False
                            break
                    elif algo_name == 'metaphone':
                        if var_codes['metaphone'] != target_codes['metaphone']:
                            matches = False
                            break
                    elif algo_name == 'nysiis':
                        if var_codes['nysiis'] != target_codes['nysiis']:
                            matches = False
                            break
                ...
                if matches:
                    ...
                    if 0.6 <= score <= 0.79:
                        medium_range_candidates.append((var, score))
                        if len(medium_range_candidates) >= count:
                            if verbose:
                                print(f"Generated {len(candidates)} candidates, found {len(medium_range_candidates)}/{count} in Medium range - STOPPING EARLY")
                            break
        ...
        # If not enough, increase depth
        if len(medium_range_candidates) < count:
            max_depth += 1
            max_candidates = min(max_candidates * 2, 10000000)
        else:
            break
    ...
    if len(medium_range_candidates) >= count:
        ...
        medium_range_candidates.sort(key=lambda x: abs(x[1] - 0.695))  # Sort by closeness to target
        result = [var for var, score in medium_range_candidates[:count]]
    else:
...
def generate_far_variations(original: str, count: int, target_score: float = 0.45, tolerance: float = 0.3, verbose: bool = False) -> List[str]:
    ...
    target_candidates = count * 1000  # Generate 1000x more than needed - try everything
    far_range_candidates = []  # Track candidates in Far range (0.3-0.59)
    
    while len(far_range_candidates) < count and max_depth <= 10:  # Stop when we have enough in Far range
        if verbose:
        ...
        for var in candidates:
            ...
            # Check if in Far range (0.3-0.59) - this is what we need
            if 0.3 <= score <= 0.59:
                far_range_candidates.append((var, score))
                if len(far_range_candidates) >= count:
                    if verbose:
                        print(f"Generated {len(candidates)} candidates, found {len(far_range_candidates)}/{count} in Far range - STOPPING EARLY")
        ...
        # If not enough in Far range, increase depth and try again - NO LIMITS
        if len(far_range_candidates) < count:
            max_depth += 1
            max_candidates = min(max_candidates * 2, 10000000)  # Very high limit - try everything
        else:
            break  # Found enough in Far range, stop searching
    ...
    if len(far_range_candidates) >= count:
        ...
        far_range_candidates.sort(key=lambda x: abs(x[1] - 0.445))  # Sort by closeness to target
        result = [var for var, score in far_range_candidates[:count]]
    else:
        # Fallback: Filter from wider scored_candidates
        far_range = []
        other_range = []
