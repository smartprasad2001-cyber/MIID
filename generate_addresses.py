...
def generate_addresses_for_country(country: str, per_country: int = 15, verbose: bool = True) -> List[str]:
    ...
    if verbose:
        print(f"  📍 Starting with {len(center_cities)} cities as center points (progressive expansion: 20 → 60 → all)")
    ...
    # 2) Progressive city expansion: Try 20 cities first, then expand to 60, then all
    city_batch_start = 0
    city_batch_size = 20
    city_batch_end = min(city_batch_size, len(cities_sorted))
    
    while len(results) < per_country and city_batch_start < len(cities_sorted):
        # Get current batch of cities
        current_batch = cities_sorted[city_batch_start:city_batch_end]
        
        if not current_batch:
            break
            
        if verbose:
            if city_batch_start == 0:
                print(f"  🏙️  Tier 2: Trying Overpass around first {len(current_batch)} cities (batch 1/3)...")
            elif city_batch_end <= 60:
                print(f"  🏙️  Tier 2: Expanding to {len(current_batch)} more cities (batch 2/3, total: {city_batch_end} cities)...")
            else:
                print(f"  🏙️  Tier 2: Using remaining {len(current_batch)} cities (batch 3/3, total: {len(cities_sorted)} cities)...")
    
        for city_idx, city in enumerate(current_batch, 1):
            if len(results) >= per_country:
                break
            ...
            if verbose:
                print(f"  📍 City {city_batch_start + city_idx}/{len(cities_sorted)}: {city_name} ({lat:.4f}, {lon:.4f})")
            nodes = fetch_nodes_from_overpass_bbox(bbox, verbose=verbose)
            stats["overpass_queries"] += 1
            stats["nodes_fetched"] += len(nodes)
        ...
        # After processing current batch, check if we need to expand to next batch
        if len(results) >= per_country:
            break  # Got enough addresses, exit city expansion loop
        
        # Expand to next batch: 20 → 60 → all
        if city_batch_end < 60:
            # Expand to 60 cities (batch 2)
            city_batch_start = city_batch_end
            city_batch_end = min(60, len(cities_sorted))
        elif city_batch_end < len(cities_sorted):
            # Expand to all remaining cities (batch 3)
            city_batch_start = city_batch_end
            city_batch_end = len(cities_sorted)
        else:
            # All cities processed, exit loop
            break
