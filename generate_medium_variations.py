"""
Generate Medium Similarity Variations (0.6-0.8, targeting ~0.7)
Uses known weights to find variations where specific algorithms match/don't match
"""
...
def generate_candidates(original: str) -> List[str]:
    """Generate candidate variations using multiple strategies."""
    candidates = []
    tested: Set[str] = set()
    vowels = ['a', 'e', 'i', 'o', 'u']
    ...
    # Strategy 7: Remove multiple letters (for Far similarity)
    if len(original) > 3:
        for i in range(len(original)):
...
def find_medium_variations(original: str, target_score: float = 0.7, count: int = 15, tolerance: float = 0.1) -> List[Tuple[str, float, Dict[str, bool]]]:
    """
    Find variations that score in Medium range (0.6-0.8, targeting ~0.7).
    
    Args:
        original: Original name
        target_score: Target score (default 0.7)
        count: Number of variations needed
        tolerance: Acceptable range around target (default 0.1, so 0.6-0.8)
    ...
    # Test candidates and find ones in Medium range
    medium_variations = []
    ...
    for var in candidates:
        ...
        if min_score <= score <= max_score:
            medium_variations.append((var, score, match_pattern))
    
    # Sort by closeness to target
    medium_variations.sort(key=lambda x: abs(x[1] - target_score))
    
    # Remove duplicates
    result = []
    seen = set()
    for var, score, pattern in medium_variations:
        if var not in seen:
...
def display_results(original: str, variations: List[Tuple[str, float, Dict[str, bool]]]):
    ...
    print("="*80)
    print(f"MEDIUM VARIATIONS FOR: '{original}'")
    print("="*80)
    print()
    ...
    for algo, weight in zip(selected_algorithms, weights):
        print(f"  {algo:10s}: {weight:.4f} ({weight*100:.1f}%)")
    print()
    print(f"Found {len(variations)} Medium variations (0.6-0.8 range):")
    print("-" * 80)
    print()
    ...
    for i, (var, score, pattern) in enumerate(variations, 1):
        ...
        print(f"{i:2d}. '{var:25s}' → Score: {score:.4f} (Medium)")
    ...
    print("="*80)
    print()
    print(f"✅ Generated {len(variations)} Medium variations for '{original}'")
    print("✅ All variations score in 0.6-0.8 range (Medium similarity)")
    print("✅ Score is calculated as weighted sum of matching algorithms")
    print()

if __name__ == "__main__":
    print("="*80)
    print("GENERATING MEDIUM SIMILARITY VARIATIONS (0.6-0.8)")
    print("="*80)
    print()
    ...
    for word in test_words:
        print("\n" + "="*80)
        variations = find_medium_variations(word, target_score=0.7, count=15, tolerance=0.1)
        
        if len(variations) >= 10:
            display_results(word, variations[:15])
            break
        else:
            print(f"⚠️  '{word}': Found {len(variations)} Medium variations (need 15)")
    ...
    test_word = "John"
    variations = find_medium_variations(test_word, target_score=0.7, count=15, tolerance=0.1)
