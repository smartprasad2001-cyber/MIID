"""
Test synapse with 10 complex names using prime_generator.py
Generates variations and evaluates scores using rewards.py
"""
...
print("="*80)
print("SYNAPSE TEST: 10 COMPLEX NAMES")
print("="*80)
print()
...
for i, test_name in enumerate(complex_names, 1):
    print(f"\n{'='*80}")
    print(f"PROCESSING {i}/10: {test_name}")
    print(f"{'='*80}\n")
    ...
    try:
        ...
        # Store variations
        all_variations[test_name] = variations
        ...
        print(f"\n✅ Completed {i}/10: Final Score = {final_score:.4f}")
...
print("\n" + "="*80)
print("SYNAPSE SUMMARY - ALL 10 COMPLEX NAMES")
print("="*80)
print()

print(f"{'Name':<30} {'Final Score':<15} {'Base Score':<15} {'Variations':<12} {'Similarity'}")
print("-" * 90)

for result in results:
    if "error" in result:
        print(f"{result['name']:<30} {'ERROR':<15} {'ERROR':<15} {'ERROR':<12} {'ERROR'}")
    else:
        name = result['name']
        ...
        print(f"{name:<30} {final:<15.4f} {base:<15.4f} {count:<12} {sim_str}")
...
if results:
    ...
    if successful:
        ...
        if len(successful) > 0:
            ...
            print(f"⚠️  Worst Score: {worst['name']} ({worst['final_score']:.4f})")
        
        # Show sample variations for best and worst
        if len(successful) > 0:
            print()
            print("="*80)
            ...
            print("="*80)
            print()
            print(f"Best performing name ({best['name']}) - Sample variations:")
            for i, var in enumerate(best['variations'][:5], 1):
                print(f"  {i}. {var}")
            print()
            print(f"Worst performing name ({worst['name']}) - Sample variations:")
            for i, var in enumerate(worst['variations'][:5], 1):
                print(f"  {i}. {var}")
