#!/usr/bin/env python3
"""
Test all Guyana addresses from zero_addresses_cache.json
to verify they pass all 4 validation phases using rewards.py.
"""
...
# Set USER_AGENT environment variable (required by Nominatim)
...
def main():
    ...
    addresses = cache_data.get('addresses', {}).get('Guyana', [])
    
    if not addresses:
        print("❌ No Guyana addresses found in cache")
        return
    
    print("=" * 80)
    print(f"TESTING {len(addresses)} GUYANA ADDRESSES")
    print("=" * 80)
    ...
    for idx, addr_obj in enumerate(addresses, 1):
        ...
        country = addr_obj.get('country', 'Guyana')
